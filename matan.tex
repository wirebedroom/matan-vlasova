...
\begin{definition}
    Функция $f : \mathbb{N} \to X$, областью определения которой является множество натуральных чисел, называется последовательностью элементов из множества $X$ и обозначается $\{x_n\}_{n=1}^{\infty}$. Приведем формально-логическую запись этого определения:
    \[\forall n\in \mathbb{N}:f(n) = x_{n} \quad \{x_n\}_{n=1}^{\infty} \subset X.\] 

	Если $X \subset \mathbb{R}$, то $f:\mathbb{N}\rightarrow \mathbb{R}$ --- числовая последовательность, т.е. $x_n \in \mathbb{R}$.
\end{definition}
\begin{exmp}
